 ...
 *
 * 1．题⽬目是在⼀一个坐标系⾥里里给很多离散的点，找到⼀一条线将所有点分成两个数量量相等的集合（在给
 定的所有点⾥里里⾯面找两个点恰好把剩余所有点分成两组），不不⽤用考虑奇偶性或者多个点在⼀一条线上
 的特殊情况。
 2. [LeetCode,218]A city's skyline is the outer contour of the silhouette formed by all the
 buildings in that city when viewed from a distance. Now suppose you are given the
 locations and height of all the buildings as shown on a cityscape photo (Figure A), write a
 program to output the skyline formed by these buildings collectively (Figure B).
 The geometric information of each building is represented by a triplet of integers [Li, Ri,
 Hi], where Li and Ri are the x coordinates of the left and right edge of the ith building,
 respectively, and Hi is its height. It is guaranteed that 0 ≤ Li, Ri ≤ INT_MAX, 0 < Hi ≤
 INT_MAX, and Ri - Li > 0. You may assume all buildings are perfect rectangles grounded
 ...
 line segment. Note that the last key point, where the rightmost building ends, is merely
 used to mark the termination of the skyline, and always has zero height. Also, the ground
 ...
 [...[2 3], [4 5], [12 7], ...]
 3.题⽬目是有⼀一张⻓长凳⼀一开始分散的坐着⼀一些⼈人，每个新来的都想坐在最宽敞的⼀一段的中间位置，
 问：如何模拟这⼀一过程
 follow up 是如果有多个⻓长凳该怎么办？以及如果⻓长凳的数据太⼤大，内存装不不下⼜又该怎么办？
 地址：
 http://www.1point3acres.com/bbs/forum.php?mod=viewthread&tid=426986&extra=page%
